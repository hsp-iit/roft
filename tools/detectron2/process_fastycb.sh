...
for OBJECT_NAME in 003_cracker_box 004_sugar_box 005_tomato_soup_can 006_mustard_bottle 009_gelatin_box 010_potted_meat_can; do
    python ./tools/detectron2/inference.py --training-dataset-name ycbv_bop_pbr --dataset-name fastycb --dataset-path $FASTYCB_PATH --gpu-id $GPU_ID --output-path $FASTYCB_PATH --object-name $OBJECT_NAME --rgb-format png
done
